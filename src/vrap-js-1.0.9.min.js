 ...
 * Released under the MIT license
 */
vRap.Actions.define("Base.primitives.Model",function(){var a;return a=vRap.Locale.alertMessages,{extend:"Base.primitives.Foundation",properties:{type:"model"},_boot:function(){var b=this,c=new $.Deferred;return b.properties.data||b.config.url||b.config.api&&!$.isEmptyObject(b.config.api)?(b.config.url&&"function"===$.type(b.config.url)&&(b.config.url=b.config.url()),c.resolve()):vRap.Msg.alert(a.noApiOrData+" | "+b._objectNamespace),c.promise()},_ajaxCall:function(b,c,d,e,f){var g,h,i,j,k=this;if(k.properties.booted){if(g=k.config.api&&k.config.api[b]?k.config.api[b]:!1,k.config.sendJSON&&(d=JSON.stringify(d)),k.config.url)if(f)for(var l in f)j=k.config.url.replace("{"+l+"}",f[l]);else j=k.config.url.replace(/{(.*?)}\/|{(.*?)}/g,"");return j||g?(h={url:g?g.url:j,method:g?g.method:"sendData"===b&&k.config.sendMethod?k.config.sendMethod:c,data:d?d:null},k.config.ajaxConf.method&&delete k.config.ajaxConf.method,$.extend(h,k.config.ajaxConf),i=$.ajax(h),$.when(i).done(function(a,c,f){k._updateModel(b,"read"===b?a:d?$.extend(d,a):a,e)}),i):k._updateModel(b,d,e)}vRap.Msg.alert(a.noApiUrl+" | "+k._objectNamespace)},_updateModel:function(a,b,c){var d,e=this,f=new $.Deferred,g="push";return d=b?b:e.properties.data,"read"===a?e.properties.data=d:"create"===a?"array"===$.type(e.properties.data)&&(e.config.prependRecord&&(g="unshift"),e.properties.data[g](d)):"update"===a?$.each(e.properties.data,function(a,c){c.id===b.id&&$.extend(e.properties.data[a],b)}):"delete"===a&&$.each(e.properties.data,function(a,c){c.id===b.id&&elf.properties.data.splice(a,1)}),c?$.when(c(b)).done(function(){e.publish("dataChange",e.properties.data,a),f.resolve(e.properties.data)}):(e.publish("dataChange",d,a),f.resolve(d)),f},getData:function(a,b,c){var d=this;return d._ajaxCall("read","GET",a,b,c)},sendData:function(a,b){var c=this;return c._ajaxCall("sendData","POST",a||c.properties.data,null,b)},sendRecord:function(a,b){var c,d,e=this;return a&&a.id?(c="update",d="PUT",e.config.forceParamId||(e.config.url=e.config.url+"/"+a.id,delete a.id)):(c="create",d="POST"),e._ajaxCall(c,d,a,null,b)},deleteRecord:function(a,b){var c=this,d=null;return c.config.forceParamId?d={id:a}:c.config.url=c.config.url+"/"+a,c._ajaxCall("delete","DELETE",d,null,b)}}}(),{}),/**
 * This file is part of vRap.JS JavaScript Framework v1.0.4
 *
 * Copyright 2015, Jonathan Mauricio Sánchez Sanabria
 * Released under the MIT license
...
//# sourceMappingURL=vrap-js-1.0.9.min.js.map