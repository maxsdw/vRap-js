 ...
 * Released under the MIT license
 */
vRap.Actions.define("Base.primitives.Model",function(){var a;return a=vRap.Locale.alertMessages,{extend:"Base.primitives.Foundation",properties:{type:"model"},_boot:function(){var b=this,c=new $.Deferred;return b.properties.data||b.config.url||b.config.api&&!$.isEmptyObject(b.config.api)?(b.config.url&&"function"===$.type(b.config.url)&&(b.config.url=b.config.url()),c.resolve()):vRap.Msg.alert(a.noApiOrData+" | "+b._objectNamespace),c.promise()},_ajaxCall:function(b,c,d,e,f){var g,h,i,j=this;if(j.properties.booted){if(g=j.config.api&&j.config.api[b]?j.config.api[b]:!1,j.config.sendJSON&&(d=JSON.stringify(d)),j.config.url)if(f)for(var k in f)j.config.parsedUrl=j.config.parsedUrl.replace("{"+k+"}",f[k]);else j.config.parsedUrl=j.config.parsedUrl.replace(/{(.*?)}\/|{(.*?)}/g,"");return j.config.parsedUrl||g?(h={url:g?g.url:j.config.parsedUrl,method:g?g.method:"sendData"===b&&j.config.sendMethod?j.config.sendMethod:c,data:d?d:null},j.config.ajaxConf.method&&delete j.config.ajaxConf.method,$.extend(h,j.config.ajaxConf),i=$.ajax(h),$.when(i).done(function(a,c,f){j._updateModel(b,"read"===b?a:d?$.extend(d,a):a,e)}),i):j._updateModel(b,d,e)}vRap.Msg.alert(a.noApiUrl+" | "+j._objectNamespace)},_updateModel:function(a,b,c){var d,e=this,f=new $.Deferred,g="push";return d=b?b:e.properties.data,"read"===a&&(e.properties.data=d),e.config.preventUpdate&&("create"===a?"array"===$.type(e.properties.data)&&(e.config.prependRecord&&(g="unshift"),e.properties.data[g](d)):"update"===a?$.each(e.properties.data,function(a,c){c.id===b.id&&$.extend(e.properties.data[a],b)}):"delete"===a&&$.each(e.properties.data,function(a,c){c.id===b.id&&elf.properties.data.splice(a,1)})),c?$.when(c(b)).done(function(){e.publish("dataChange",e.properties.data,a),f.resolve(e.properties.data)}):(e.publish("dataChange",d,a),f.resolve(d)),f},getData:function(a,b,c){var d=this;return d.config.parsedUrl=d.config.url,d._ajaxCall("read","GET",a,b,c)},sendData:function(a,b){var c=this;return c.config.parsedUrl=c.config.url,c._ajaxCall("sendData","POST",a||c.properties.data,null,b)},sendRecord:function(a,b){var c,d,e=this;return e.config.parsedUrl=e.config.url,a&&a.id?(c="update",d="PUT",e.config.forceParamId||(e.config.parsedUrl=e.config.url+"/"+a.id,delete a.id)):(c="create",d="POST"),e._ajaxCall(c,d,a,null,b)},deleteRecord:function(a,b){var c=this,d=null;return c.config.parsedUrl=c.config.url,c.config.forceParamId?d={id:a}:c.config.parsedUrl=c.config.url+"/"+a,c._ajaxCall("delete","DELETE",d,null,b)}}}(),{}),/**
 * This file is part of vRap.JS JavaScript Framework v1.0.4
 *
 * Copyright 2015, Jonathan Mauricio Sánchez Sanabria
 * Released under the MIT license
 */
vRap.Actions.define("Base.primitives.View",function(){var a;return a=vRap.Locale.alertMessages,{extend:"Base.primitives.Foundation",properties:{type:"view"},_boot:function(){var b,c,d=this,e=new $.Deferred;return b=d.properties.domEl,b?b instanceof jQuery&&0!==b.length||(vRap.Msg.alert(a.badDomEl+" | "+d._objectNamespace),e.reject()):(b=d.properties.domEl=$('<div id="'+d.properties.id+'"></div>'),c=d.properties.insertTo?d.properties.insertTo:$("body"),c instanceof jQuery&&c.length>0?c.append(b):(vRap.Msg.alert(a.badInsertEl+" | "+d._objectNamespace),e.reject())),"rejected"!==e.state()&&(d.properties.style&&b.attr("class",d.properties.style),$.when(d._setTemplate(),d._setReactComponents,d._setModel(),d._setEvents()).done(function(){e.resolve()})),d.properties.hidden&&b.addClass("hide"),e.promise()},_setTemplate:function(){var a=this,b=new $.Deferred;return a.config.template?$.get(a.config.template,function(c){a.properties.template=$(c).html(),b.resolve()}):b.resolve(),b.promise()},_setReactComponents:function(a){var b,c=this,d=vRap.Query.getApp(vRap.Properties.activeApp),e=new $.Deferred;b=a||c.config.components,b&&"array"===$.type(b)&&(c.reactElements=c.reactElements||{},c.reactInstances=c.reactElements||{},$.each(b,function(a,b){c.reactElements[b.reactClass]=React.createElement(d.reactCmpts[b.reactClass],$.extend(b.objProps,{locale:c.config.locale?d.localeObj[c.config.locale]:{},viewNamespace:c._objectNamespace}),b.children)})),e.resolve()},_setModel:function(){var a,b=this,c=new $.Deferred,d=[];b.linked.model=b.linked.model||{},b.properties.model&&(a=$.type(b.properties.model),"string"===a?d.push(b.properties.model):"array"===a&&(d=b.properties.model),$.each(d,function(a,c){var d,e;d=c.split("."),e=d[d.length-1],b.linked.model[e]=vRap.Query.getObj(c),b.linked.model[e]&&(b.linked.model[e].subscribe(function(a,c,d){"dataChange"===a&&b.refresh&&b.refresh(c,d)}),delete b.properties.model)})),c.resolve()},_setEvents:function(){var a=this,b=new $.Deferred;a.config.events&&!$.isEmptyObject(a.config.events)&&$.each(a.config.events,function(b,c){var d,e;d=b.substr(0,b.indexOf(" ")),e=b.substr(b.indexOf(" ")+1),a.properties.domEl.on(d,e,function(b){a[c]($(this),c,b)})}),b.resolve()},processComponents:function(a){var b=this,c=new $.Deferred;return $.when(b._setReactComponents(a)).done(function(){c.resolve()}),c.promise()},hide:function(){this.properties.domEl.addClass("hide")},show:function(){this.properties.domEl.removeClass("hide")},emit:function(a,b){var c=this;c.publish(a,b)}}}(),{});
//# sourceMappingURL=vrap-js-1.1.1.min.js.map