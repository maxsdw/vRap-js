...
$.each(b,function(a,c){$.isFunction(b[a])||("properties"===a?$.extend(e.properties,b[a]):e[a]=c)});for(f in c)"function"===$.type(c[f])?e[f]=c[f]:e.properties[f]=c[f]},h=h[e]}),e=h,d&&(e.prototype=Object.create(d.prototype),e.prototype.constructor=e),$.each(b,function(a,c){$.isFunction(b[a])&&(e.prototype[a]=c)}),$.isEmptyObject(c)||$.each(c,function(a,b){e[a]=b})},create:function(a,b,c){var d,e,f,g,h,i=this,j=new $.Deferred;return"object"===$.type(a)&&a.instances||a.interfaces?$.when(i._classIterator(a)).done(function(){j.resolve()}):(e=vRap.Query.getObj(b),e?j.resolve(e):(f=vRap.Query.getClass(a),f&&(d=vRap.Query.getApp(vRap.Properties.activeApp),d&&(g=b.split("."),h=d.objManager,$.each(g,function(a,b){a<g.length-1?h[b]||(h[b]={}):h[b]=new f(c),h=h[b]}),h._objectNamespace=b,h.properties.id="vRap_"+b.toLowerCase().replace(/\./g,"_")+"_"+vRap.Generators.genIdNumber(),h.properties.alias&&(d.references.usedAlias.indexOf(h.properties.alias)<0?d.references.usedAlias.push(h.properties.alias):vRap.Msg.alert(vRap.Locale.alertMessages.duplicatedAlias+" | Object: "+b+" | Alias: "+h.properties.alias)),$.when(h._boot()).done(function(){h.properties.booted=!0,h.init?$.when(h.init()).done(function(a){h.properties.initialized=!0,j.resolve(h,a)}):vRap.Msg.alert(vRap.Locale.alertMessages.noInit+" | "+b)}))))),j.promise()},destroy:function(a){var b,c,d,e,f,g=new $.Deferred,h=0;return b=vRap.Query.getApp(vRap.Properties.activeApp),c=vRap.Query.getObj(a),d=a.split("."),e=function(b,c){b[c]._objectNamespace&&b[c]._objectNamespace===a?(b[c]=null,delete b[c],g.resolve()):e(b[c],d[h+=1])},c?("Base.primitives.Controller"===c._extend&&c.linked&&(delete b.references.controllers[c.properties.alias],$.each(c.linked,function(a,b){$.each(b,function(a,b){self.destroy(b._objectNamespace)})})),c.properties.domEl&&(c.reactElements&&Object.keys(c.reactElements).length>0&&ReactDOM.unmountComponentAtNode(c.properties.domEl[0]),c.properties.domEl.remove()),c.properties.alias&&(f=b.references.usedAlias.indexOf(c.properties.alias),f>=0&&b.references.usedAlias.splice(f,1)),e(b.objManager,d[h])):g.resolve(),g.promise()},destroyByClass:function(a){$.each(vRap.Query.getObjsByClass(a),function(a,b){vRap.Actions.destroy(b._objectNamespace)})},addCustomListener:function(a){var b=vRap.Query.getObj(a.component).domEl;b.off(a.eventType),b.on(a.eventType,a.method)},_classIterator:function(a){var b,c=new $.Deferred,d=0,e=!1,f=!1,g=[];return b=function(h,i){var j;h&&h.length>0&&h.length>d?(j=h[d],a.async?($.each(a.instances,function(a,b){g.push(vRap.Actions.create(b["class"],b.namespace,b.properties)),d+=1}),$.when.apply($,g).done(function(){b(h,i)})):$.when(vRap.Actions.create(j["class"],j.namespace,j.properties)).done(function(){d+=1,b(h,i)})):("instances"===i?e=!0:"interfaces"===i&&(f=!0),f?c.resolve():(d=0,b(a.interfaces,"interfaces")))},b(a.instances,"instances"),c.promise()}},Msg:{alert:function(a){console.log("%c "+a,"color: red")}}};window.vRap||(window.vRap=a)}(),vRap.Locale={alertMessages:{noApp:"vRap could not find the App instance, create it using the method vRap.newApp().",noNamespace:"Bad namespace specification or namespace doesn't exist.",noInit:"The object constructor specified does not contain an init() method.",noObject:"Object couldn't be found in this application.",badDomEl:"Specified domEl is not a valid jquery selector or the element doesn't exist.",badInsertEl:"Specified InsertTo element is not a valid jquery selector or the element doesn't exist.",noApiOrData:"You must specify an API Url, API configuration or include local data",noApiUrl:"You must specify an API Url inside the model definition",noSendData:"You must include the data to be sent to the server",noAlias:"Controllers and all referenced instances inside them must include an alias",duplicatedAlias:"The alias asigned to this object already exists, please pick a different one"}},/**
 * This file is part of vRap.JS JavaScript Framework v1.0.4
 *
 * Copyright 2015, Jonathan Mauricio Sánchez Sanabria
 * Released under the MIT license
 */
vRap.Actions.define("Base.primitives.Foundation",function(){return{_boot:function(){var a=new $.Deferred;return a.resolve()},subscribe:function(a){this.observerList.push(a)},unsubscribe:function(a){var b,c=$.type(a);"string"===c?this.observerList=$.grep(this.observerList,function(b,c){return b.key===a?!1:!0}):"function"===c&&(b=this.observerList.indexOf(a,0),b>=0&&this.observerList.splice(b,1))},publish:function(a,b,c,d,e){$.each(this.observerList,function(f,g){var h=$.type(g);"object"===h?g.callback(a,b,c,d,e):"function"===h&&g(a,b,c,d,e)})}}}(),{}),/**
 * This file is part of vRap.JS JavaScript Framework v1.0.4
 *
 * Copyright 2015, Jonathan Mauricio Sánchez Sanabria
 * Released under the MIT license
 */
vRap.Actions.define("Base.primitives.Controller",function(){var a;return a=vRap.Locale.alertMessages,{extend:"Base.primitives.Foundation",properties:{type:"controller"},_boot:function(){var b=this,c=new $.Deferred;return b.properties.alias?(b.linked={model:{},view:{}},b._setModels().then(function(){b._setViews().then(function(){b._setListeners().done(function(){var a=vRap.Query.getApp(vRap.Properties.activeApp);a.references.controllers[b.properties.alias]=b,c.resolve()})})})):(vRap.Msg.alert(a.noAlias+" | "+b._objectNamespace),c.reject()),c.promise()},_setModels:function(){var a=this,b=new $.Deferred;return $.when(a._classIterator(a.config.models,"model")).done(function(){b.resolve()}),b.promise()},_setViews:function(a){var b=this,c=new $.Deferred;return $.when(b._classIterator(a||b.config.views,"view")).done(function(){c.resolve()}),c.promise()},_setListeners:function(){var a=this,b=new $.Deferred,c=0;return a.config.listeners&&!$.isEmptyObject(a.config.listeners)?$.each(a.config.listeners,function(d,e){$.each(a.linked.view,function(b,c){c.subscribe(function(b,c){b===d&&a[e](b,c)})}),c+=1,Object.keys(a.config.listeners).length===c&&b.resolve()}):b.resolve(),b.promise()},_classIterator:function(b,c){var d=this,e=new $.Deferred,f=0;return b&&!$.isEmptyObject(b)?"array"===$.type(b)?($.each(b,function(b,e){var f=vRap.Query.getObj(e);f&&(f.properties.alias?d.linked[c][f.properties.alias]=f:vRap.Msg.alert(a.noAlias+" | "+f._objectNamespace))}),e.resolve()):$.each(b,function(g,h){var i,j=$.extend({},h);i=j.namespace,delete j.namespace,j.domEl&&(j.domEl=$(j.domEl.selector)),$.when(vRap.Actions.create(g,i,j)).done(function(g){f+=1,j.alias?(d.linked[c][j.alias]=g,Object.keys(b).length===f&&e.resolve()):vRap.Msg.alert(a.noAlias+" | "+g._objectNamespace)})}):e.resolve(),e.promise()},processViews:function(a){var b=this;b._setViews(a).then(function(){b._setListeners()})},showViews:function(a){var b=this;return a?(b.linked.view[a].show(),b.onShowView&&b.onShowView(a)):_.each(b.linked.view,function(a){a.show()}),b},hideViews:function(a){var b=this;return a?(b.linked.view[a].hide(),b.onHideView&&b.onHideView(a)):_.each(b.linked.view,function(a){a.hide()}),b},destroyItself:function(){vRap.Actions.destroy(this._objectNamespace)}}}(),{}),/**
 * This file is part of vRap.JS JavaScript Framework v1.0.4
 *
 * Copyright 2015, Jonathan Mauricio Sánchez Sanabria
 * Released under the MIT license
 */
vRap.Actions.define("Base.primitives.Interface",function(){return{extend:"Base.primitives.Foundation",properties:{type:"interface"},observerList:[],controller:{},_boot:function(){var a=this,b=new $.Deferred;a.controller=vRap.Query.getApp(vRap.Properties.activeApp).references.controllers,b.resolve()},hideViews:function(a){var b,c=this;b=a||c.config.controllers,b&&b.forEach(function(a){c.controller[a]&&c.controller[a].hideViews()})}}}(),{}),/**
 * This file is part of vRap.JS JavaScript Framework v1.0.4
 *
 * Copyright 2015, Jonathan Mauricio Sánchez Sanabria
 * Released under the MIT license
 */
vRap.Actions.define("Base.primitives.Model",function(){var a;return a=vRap.Locale.alertMessages,{extend:"Base.primitives.Foundation",properties:{type:"model"},_boot:function(){var b=this,c=new $.Deferred;return b.properties.data||b.config.url||b.config.api&&!$.isEmptyObject(b.config.api)?(b.config.url&&"function"===$.type(b.config.url)&&(b.config.url=b.config.url()),c.resolve()):vRap.Msg.alert(a.noApiOrData+" | "+b._objectNamespace),c.promise()},_ajaxCall:function(b,c,d,e,f,g,h){var i,j,k,l=this,m="?";if(l.properties.booted){if(i=l.config.api&&l.config.api[b]?l.config.api[b]:!1,l.config.sendJSON&&(d=JSON.stringify(d)),l.config.url){if(f)for(var n in f)l.config.parsedUrl=l.config.parsedUrl.replace("{"+n+"}",null===f[n]?"":f[n]);l.config.parsedUrl=l.config.parsedUrl.replace(/{(.*?)}\/|{(.*?)}/g,"").replace(/([^:]\/)\/+/g,"$1"),l.config.forceQueryParams&&"object"===$.type(l.config.forceQueryParams)&&($.each(l.config.forceQueryParams,function(a,b){m+=a+"="+b}),l.config.parsedUrl+=m)}return l.config.parsedUrl||i?(j={url:i?i.url:l.config.parsedUrl,method:i?i.method:"sendData"===b&&l.config.sendMethod?l.config.sendMethod:c,data:d?d:null,context:h},l.config.dynamicHeaders&&(j.headers=l.config.dynamicHeaders[j.method]),l.config.ajaxConf.method&&delete l.config.ajaxConf.method,$.extend(j,l.config.ajaxConf),k=$.ajax(j),$.when(k).done(function(a,c,f){l._updateModel(b,"read"===b?a:d?$.extend(d,a):a,e,g,this)}),k):l._updateModel(b,d,e,g)}vRap.Msg.alert(a.noApiUrl+" | "+l._objectNamespace)},_updateModel:function(a,b,c,d,e){var f,g=this,h=new $.Deferred,i="push";return f=b?b:g.properties.data,"read"===a&&(g.properties.data=f),g.config.preventUpdate||("create"===a?"array"===$.type(g.properties.data)&&(g.config.prependRecord&&(i="unshift"),g.properties.data[i](f)):"update"===a?$.each(g.properties.data,function(a,c){c.id===b.id&&$.extend(g.properties.data[a],b)}):"delete"===a&&$.each(g.properties.data,function(a,c){c.id===b.id&&g.properties.data.splice(a,1)})),c?$.when(c(b,g)).done(function(){d||g.publish("dataChange",g.properties.data,a,g.properties.alias,e),h.resolve(g.properties.data)}):(d||g.publish("dataChange",f,a,g.properties.alias,e),h.resolve(f)),h.promise},getData:function(a,b,c,d,e){return this.config.parsedUrl=this.config.url,this._ajaxCall("read","GET",a,b,c,d,e)},sendData:function(a,b,c,d){return this.config.parsedUrl=this.config.url,this._ajaxCall("sendData","POST",a||this.properties.data,null,b,c,d)},sendRecord:function(a,b,c,d,e,f){var g=this,h={};return g.config.parsedUrl=g.config.url,a&&a.id?(h={action:"update",method:"PUT"},g.config.forceParamId||(g.config.parsedUrl=g.config.url+"/"+a.id)):h={action:"create",method:"POST"},d&&"object"===$.type(d)&&(h=d),c&&(a=JSON.stringify(a)),g._ajaxCall(h.action,h.method,a,null,b,e,f)},deleteRecord:function(a,b,c,d){var e=this,f=null;return e.config.parsedUrl=e.config.url,a&&(e.config.forceParamId?f={id:a}:e.config.parsedUrl=e.config.url+"/"+a),e._ajaxCall("delete","DELETE",f,null,b,c,d)}}}(),{}),/**
 * This file is part of vRap.JS JavaScript Framework v1.0.4
 *
 * Copyright 2015, Jonathan Mauricio Sánchez Sanabria
 * Released under the MIT license
 */
vRap.Actions.define("Base.primitives.View",function(){var a;return a=vRap.Locale.alertMessages,{extend:"Base.primitives.Foundation",properties:{type:"view"},_boot:function(){var b,c,d=this,e=new $.Deferred;return b=d.properties.domEl,b?b instanceof jQuery&&0!==b.length||(vRap.Msg.alert(a.badDomEl+" | "+d._objectNamespace),e.reject()):(b=d.properties.domEl=$('<div id="'+d.properties.id+'"></div>'),c=d.properties.insertTo?d.properties.insertTo:$("body"),c instanceof jQuery&&c.length>0?c.append(b):(vRap.Msg.alert(a.badInsertEl+" | "+d._objectNamespace),e.reject())),"rejected"!==e.state()&&(d.properties.style&&b.attr("class",d.properties.style),$.when(d._setTemplate(),d._setReactComponents(),d._setModel(),d._setEvents()).done(function(){e.resolve()})),d.properties.hidden&&b.addClass("hide"),e.promise()},_setTemplate:function(){var a=this,b=new $.Deferred;return a.config.template?$.get(a.config.template,function(c){a.properties.template=$(c).html(),b.resolve()}):b.resolve(),b.promise()},_setReactComponents:function(a){var b,c=this,d=vRap.Query.getApp(vRap.Properties.activeApp),e=new $.Deferred;b=a||c.config.components,b&&"array"===$.type(b)&&(c.reactElements=c.reactElements||{},c.reactInstances=c.reactElements||{},$.each(b,function(a,b){c.reactElements[b.reactClass]=React.createElement(d.reactCmpts[b.reactClass],$.extend(b.objProps,{locale:c.config.locale?d.localeObj[c.config.locale]:{},viewNamespace:c._objectNamespace}),b.children)})),e.resolve()},_setModel:function(){var a,b=this,c=new $.Deferred,d=[];b.linked.model=b.linked.model||{},b.properties.model&&(a=$.type(b.properties.model),"string"===a?d.push(b.properties.model):"array"===a&&(d=b.properties.model),$.each(d,function(a,c){var d,e;d=c.split("."),e=d[d.length-1],b.linked.model[e]=vRap.Query.getObj(c),b.linked.model[e]&&(b.linked.model[e].subscribe({key:b._objectNamespace,callback:b._observer.bind(b)}),delete b.properties.model)})),c.resolve()},_unsubscribeModel:function(){var a=this,b=new $.Deferred;$.each(a.linked.model,function(b,c){c.unsubscribe(a._objectNamespace)}),b.resolve()},_observer:function(a,b,c,d,e){"dataChange"===a&&this.refresh&&this.refresh(b,c,d,e)},_setEvents:function(){var a=this,b=new $.Deferred;a.config.events&&!$.isEmptyObject(a.config.events)&&$.each(a.config.events,function(b,c){var d,e;d=b.substr(0,b.indexOf(" ")),e=b.substr(b.indexOf(" ")+1),a.properties.domEl.on(d,e,function(b){a[c]($(this),c,b)})}),b.resolve()},processComponents:function(a){var b=this,c=new $.Deferred;return $.when(b._setReactComponents(a)).done(function(){c.resolve()}),c.promise()},hide:function(){this.properties.domEl.addClass("hide")},show:function(){this.properties.domEl.removeClass("hide")},emit:function(a,b){var c=this;setTimeout(function(){c.publish(a,b)},0)}}}(),{});
//# sourceMappingURL=vrap-js-1.2.0.min.js.map